...
# This can later be moved to an Admin Settings model.
...
def calculate_reservation_deposit(*, agreed_price):
    """
    Calculate the reservation deposit.

    Initial business rule:

        10% of the agreed price.

    This can later be replaced with configurable admin rules.
    """

    if agreed_price is None:
        raise ValidationError(
            "Bei iliyokubaliwa haijapatikana."
        )
...
def create_reservation(
    *,
    transaction,
    user,
    duration_hours=DEFAULT_RESERVATION_HOURS,
):
    """
    Create a reservation for a transaction.

    The reservation starts in PENDING_PAYMENT.

    The listing is NOT marked RESERVED until payment is confirmed.
    """
...
def confirm_reservation_payment(
    *,
    ...
    """
    Confirm reservation deposit payment.

    For now this is a service-level/manual confirmation.

    Later the payment gateway webhook will call this function.
    """
    ...
    if reservation.payment_status == Reservation.PaymentStatus.PAID:
        raise ValidationError(
            "Reservation payment tayari imelipwa."
        )
...
@db_transaction.atomic
def start_inspection_period(
    *,
    transaction,
    user,
    duration_hours=DEFAULT_INSPECTION_HOURS,
):
    """
    Start the inspection period after a successful reservation.

    The reservation remains active while the inspection is running.
    """
    ...
    transaction.buyer_decision = (
        Transaction.BuyerDecision.PENDING
    )
...
def expire_reservation(
    *,
    ...
    The listing becomes AVAILABLE again if the transaction has not
    progressed into a protected/completed state.
    """
    ...
    if reservation.status != Reservation.Status.ACTIVE:
        raise ValidationError(
            "Reservation hii haipo ACTIVE."
        )

    if not reservation.expires_at:
        raise ValidationError(
            "Reservation haina muda wa kuisha."
        )

    if reservation.expires_at > now:
        raise ValidationError(
            "Reservation bado haija-expire."
        )
    ...
    if inspection and inspection.status in [
        ...
        inspection.status = (
            InspectionPeriod.Status.CANCELLED
        )
...
@db_transaction.atomic
def expire_inspection_period(
    *,
    inspection,
):
    """
    Expire an inspection period.

    If the buyer has not submitted a decision, the transaction is
    moved to DISPUTED so it is not silently completed.
    """
    ...
    if inspection.status != InspectionPeriod.Status.ACTIVE:
        raise ValidationError(
            "Inspection hii haipo ACTIVE."
        )

    if not inspection.expires_at:
        raise ValidationError(
            "Inspection haina muda wa kuisha."
        )

    if inspection.expires_at > now:
        raise ValidationError(
            "Inspection bado haija-expire."
        )
